...
namespace tsn\tsn\cron\task;

/**
 * the-spot.net cron task.
 */
class sample extends \phpbb\cron\task\base
{
	...
	public function run()
	{
		...
		$this->config->set('tsn_cron_last_run', time(), false);
	}
	...
	{
		return $this->config['tsn_cron_last_run'] < time() - $this->cron_frequency;
	}
}
