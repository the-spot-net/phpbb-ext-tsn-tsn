...
class install_sample_data extends \phpbb\db\migration\migration
{
	...
	public function update_data()
	{
		return array(
			...
			array('config.add', array('tsn_tsn_sample_int', 0)),
			array('config.add', array('tsn_tsn_sample_str', '')),

			// Add a new config_text table setting
			array('config_text.add', array('tsn_tsn_sample', '')),

			// Add new permissions
			array('permission.add', array('a_new_tsn_tsn')), // New admin permission
			array('permission.add', array('m_new_tsn_tsn')), // New moderator permission
			array('permission.add', array('u_new_tsn_tsn')), // New user permission
			...
			array('permission.permission_set', array('ROLE_ADMIN_FULL', 'a_new_tsn_tsn')), // Give ROLE_ADMIN_FULL a_new_tsn_tsn permission
			array('permission.permission_set', array('ROLE_USER_FULL', 'u_new_tsn_tsn')), // Give ROLE_USER_FULL u_new_tsn_tsn permission
			array('permission.permission_set', array('ROLE_USER_STANDARD', 'u_new_tsn_tsn')), // Give ROLE_USER_STANDARD u_new_tsn_tsn permission
			array('permission.permission_set', array('REGISTERED', 'u_new_tsn_tsn', 'group')), // Give REGISTERED group u_new_tsn_tsn permission
			array('permission.permission_set', array('REGISTERED_COPPA', 'u_new_tsn_tsn', 'group', false)), // Set u_new_tsn_tsn to never for REGISTERED_COPPA

			// Add new permission roles
			array('permission.role_add', array('tsn admin role', 'a_', 'a new role for admins')), // New role "tsn admin role"
			array('permission.role_add', array('tsn moderator role', 'm_', 'a new role for moderators')), // New role "tsn moderator role"
			array('permission.role_add', array('tsn user role', 'u_', 'a new role for users')), // New role "tsn user role"
