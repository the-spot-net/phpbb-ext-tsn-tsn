...
class query
{
    ...
    const SQL_MYSPOT_FEED_TOPIC_INFOS = 'SELECT t.*, f.forum_id, f.forum_name, tt.mark_time, ft.mark_time as f_mark_time, pf.post_text as first_post_text, pf.bbcode_uid as first_post_bbcode_uid, pf.bbcode_bitfield as first_post_bbcode_bitfield, pl.post_text as last_post_text, pl.bbcode_uid as last_post_bbcode_uid, pl.bbcode_bitfield as last_post_bbcode_bitfield FROM ' . TOPICS_TABLE . ' t LEFT JOIN ' . POSTS_TABLE . ' pf ON (t.topic_first_post_id = pf.post_id) LEFT JOIN ' . POSTS_TABLE . ' pl ON (t.topic_last_post_id = pl.post_id) LEFT JOIN ' . FORUMS_TABLE . ' f ON (f.forum_id = t.forum_id) LEFT JOIN ' . TOPICS_TRACK_TABLE . ' tt ON (tt.user_id = ' . self::TOKEN_USER_ID . ' AND t.topic_id = tt.topic_id) LEFT JOIN ' . FORUMS_TRACK_TABLE . ' ft ON (ft.user_id = ' . self::TOKEN_USER_ID . ' AND ft.forum_id = f.forum_id) ' . ' WHERE ' . self::TOKEN_TOPIC_IDS . ' ' . self::TOKEN_FORUM_ID_EXCLUSIONS . ' AND ' . self::TOKEN_FORUM_ID_WHITELIST . ' ORDER BY t.topic_last_post_time DESC';
    const SQL_MYSPOT_FEED_MOVED_TOPIC_INFOS = 'SELECT t.*, pf.post_text as first_post_text, pf.bbcode_uid as first_post_bbcode_uid, pf.bbcode_bitfield as first_post_bbcode_bitfield, pl.post_text as last_post_text, pl.bbcode_uid as last_post_bbcode_uid, pl.bbcode_bitfield as last_post_bbcode_bitfield FROM ' . TOPICS_TABLE . ' t LEFT JOIN ' . POSTS_TABLE . ' pf ON (t.topic_first_post_id = pf.post_id) LEFT JOIN ' . POSTS_TABLE . ' pl ON (t.topic_last_post_id = pl.post_id) WHERE ' . self::TOKEN_TOPIC_IDS;
